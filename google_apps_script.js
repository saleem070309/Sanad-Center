...
function setupSheets() {
  ...
  if (settingsSheet.getLastRow() === 1) {
    settingsSheet.appendRow(['phone', '+962 7 7734 9013']);
    settingsSheet.appendRow(['facebook', 'https://web.facebook.com/profile.php?id=100063998229344&mibextid=wwXIfr']);
    settingsSheet.appendRow(['facebook_name', 'سند سنتر - الرمثا']);
  }
...
function deleteCategory(name) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim() === trimmedName) {
      sheet.deleteRow(i + 1);
      return { status: 'success', message: 'تم حذف القسم' };
    }
  }
  return { status: 'error', message: 'القسم غير موجود: ' + trimmedName };
}
